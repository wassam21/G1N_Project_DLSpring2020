...
class MonodepthDataloader(object):
    ...
    def __init__(self, data_path, filenames_file, params, dataset, mode, grndPath=""):
        self.data_path = data_path
        self.params = params
        self.dataset = dataset
        self.mode = mode
        self.grndPath = grndPath

        self.left_image_batch  = None
        self.right_image_batch = None
        self.grnd_image_batch = None
        ...
        if mode == 'test' and not self.params.do_stereo:
            ...
            grnd_image_path = tf.string_join([self.grndPath, split_line[2]])
            left_image_o  = self.read_image(left_image_path)
            right_image_o = self.read_image(right_image_path)
            grnd_image_o = self.read_image(grnd_image_path)
        ...
        if mode == 'train':
            ...
            grnd_image = tf.cond(do_augment >= 0 , lambda: tf.image.rgb_to_grayscale(grnd_image_o), lambda: grnd_image_o)
            
            left_image.set_shape( [None, None, 3])
            right_image.set_shape([None, None, 3])
            grnd_image.set_shape([None, None, 1])

            # capacity = min_after_dequeue + (num_threads + a small safety margin) * batch_size
            min_after_dequeue = 2048
            capacity = min_after_dequeue + 4 * params.batch_size
            self.left_image_batch, self.right_image_batch, self.grnd_image_batch = tf.train.shuffle_batch([left_image, right_image, grnd_image],
