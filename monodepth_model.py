...
class MonodepthModel(object):
    ...
    def __init__(self, params, mode, left, right, grnd=None, reuse_variables=None, model_index=0):
        self.params = params
        self.mode = mode
        self.left = left
        self.right = right
    ...
    def build_losses(self):
        with tf.variable_scope('losses', reuse=self.reuse_variables):
            ...
            # L2
            self.l2_left = tf.square( self.disp_left_est[0] - self.grnd)
            self.l2_disp_loss_left  = tf.reduce_mean(self.l2_left)
            self.l2_right = tf.square(self.disp_right_est[0] - self.grnd)
            self.l2_disp_loss_right = tf.reduce_mean(self.l2_right)
            
            self.l2_disp_loss = (self.l2_disp_loss_left + self.l2_disp_loss_right) / 2
            ...
            #"""
            """
            # WEIGTHED SUM With L2
            self.image_loss_right = [self.params.alpha_image_loss * self.ssim_loss_right[i] + (1 - self.params.alpha_image_loss) * self.l2_reconstruction_loss_right[i] for i in range(4)]
            self.image_loss_left  = [self.params.alpha_image_loss * self.ssim_loss_left[i]  + (1 - self.params.alpha_image_loss) * self.l2_reconstruction_loss_left[i]  for i in range(4)]
            self.image_loss = tf.add_n(self.image_loss_left + self.image_loss_right)
            """
            """
            # WEIGTHED SUM With L2 and L1 
            self.image_loss_right = [self.params.alpha_image_loss * self.ssim_loss_right[i] + ((1 - self.params.alpha_image_loss)) * (self.l2_reconstruction_loss_right[i] + self.l1_reconstruction_loss_right[i]) for i in range(4)]
            self.image_loss_left  = [self.params.alpha_image_loss * self.ssim_loss_left[i]  + ((1 - self.params.alpha_image_loss)) * (self.l2_reconstruction_loss_left[i]  + self.l1_reconstruction_loss_left[i])  for i in range(4)]
            self.image_loss = tf.add_n(self.image_loss_left + self.image_loss_right)
            #"""
            
            """
            # Addition of everything
            self.image_loss_right = [self.ssim_loss_right[i] +  (self.l2_reconstruction_loss_right[i] + self.l1_reconstruction_loss_right[i]) for i in range(4)]
            self.image_loss_left  = [self.ssim_loss_left[i]  +  (self.l2_reconstruction_loss_left[i]  + self.l1_reconstruction_loss_left[i])  for i in range(4)]
            self.image_loss = tf.add_n(self.image_loss_left + self.image_loss_right)
            """
            """
            # Getting MAX of the Two at each pixel
            self.image_loss_right = [tf.maximum(self.ssim_loss_right[i] , self.l1_reconstruction_loss_right[i]) for i in range(4)]
            self.image_loss_left  = [tf.maximum(self.ssim_loss_left[i]  , self.l1_reconstruction_loss_left[i])  for i in range(4)]
            self.image_loss = tf.add_n(self.image_loss_left + self.image_loss_right)
            """
            
            """
            # Getting MAX of the Three at each pixel
            self.image_loss_right = [tf.maximum(self.ssim_loss_right[i] , tf.maximum(self.l1_reconstruction_loss_right[i], self.l2_reconstruction_loss_right[i])) for i in range(4)]
            self.image_loss_left  = [tf.maximum(self.ssim_loss_left[i]  , tf.maximum(self.l1_reconstruction_loss_left[i] , self.l2_reconstruction_loss_left[i]))  for i in range(4)]
            self.image_loss = tf.add_n(self.image_loss_left + self.image_loss_right)
            """
            ...
            # TOTAL LOSS
            self.total_loss = self.image_loss + self.params.disp_gradient_loss_weight * self.disp_gradient_loss + self.params.lr_loss_weight * self.lr_loss + self.l2_disp_loss
            
            # Getting Max of the losses at each pixel.
            #self.total_loss = tf.maximum(self.image_loss,  self.lr_loss) + self.params.disp_gradient_loss_weight * self.disp_gradient_loss
            
            # Addition of everything (5 Losses Come together)
            #self.total_loss = self.image_loss + self.lr_loss + self.disp_gradient_loss
